...
module.exports = declare((api, options) => {
  ...
  return {
    ...
    visitor: {
      ...
      'FunctionExpression|ArrowFunctionExpression|ObjectMethod': function (
        path
      ) {
        if (doesReturnJSX(types, path.node.body)) {
          addDisplayNamesToFunctionComponent(types, path, resourcePath);
        }
      },
 ...
 */
function addDisplayNamesToFunctionComponent(types, path, resourcePath) {
  ...
  path.find((parentPath) => {
    ...
    if (parentPath.isCallExpression()) {
      ...
      if (
        types.isArrowFunctionExpression(callee) ||
  ...
  if (
    ...
    !hasBeenAssignedPrev(types, assignmentPath, pattern, name)
  ) {
    return
  }
  // skip unnecessary addition of name if it is reassigned later on
  if (hasBeenAssignedNext(types, assignmentPath, pattern)) {
    return
  }
  // skip unnecessary addition of name if it is reassigned later on
  if (hasBeenParentAssignedNext(types, assignmentPath, pattern)) {
    return
  }
...
function hasBeenAssignedNext(types, assignmentPath, pattern) {
  return assignmentPath.getAllNextSiblings && assignmentPath.getAllNextSiblings().some((sibling) => {
    const expression = sibling.get('expression')
    if (!types.isAssignmentExpression(expression.node, { operator: '=' }))
      return false
...
function hasBeenParentAssignedNext(types, assignmentPath, pattern) {
  return assignmentPath.parentPath && assignmentPath.parentPath.getAllNextSiblings().some((sibling) => {
    const expression = sibling.get('expression')
    if (!types.isAssignmentExpression(expression.node, { operator: '=' }))
      return false
    return expression.get('left').node.property.name === 'displayName';
